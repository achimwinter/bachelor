...
\parencite{hankerson_elliptic_2011} \parencite{schneier_essays_2007} \parencite{noauthor_security_nodate}

% Dass NIST-P-256 in die eigentliche Arbeit 
...
The way it works is, that a secret which has to be stored gets packaged in a keychain item. Additionally, there should be packaged public attributes like a name or an identifier to make the item searchable. However, this means that the key was handled to some point in plaintext in the phone's memory, which could lead to a compromised key, if the app is also compromised. So, just like the Android Keystore System, Apple created the Secure Enclave, which is a coprocessor doing cryptographic operations. Again, it is very important, that the main system/processor cannot see the key in plaintext. This way the key never leaves the Secure Enclave and makes it harder for an adversary to compromise the key.
Apple has implemented this feature for all smartphones with an A7 chip or higher. However, the Secure Enclave only supports NIST-P-256 elliptic curves at the moment.
\parencite{noauthor_storing_nodate} \parencite{noauthor_keychain_nodate}

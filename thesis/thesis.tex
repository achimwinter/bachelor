...
\end{figure}
%This could come due to several reasons. Whistleblower Edward Snowden has received a lot of media interest and has made people aware of using secure services on %the Internet, when he revealed the capabilities of the National Security Agency (NSA). 
\newpage
...
The Key Distribution Center exists to allow users to upload a bunch of Key Bundles so that they can be used by other users and in addition, messages that cannot be delivered at this point, because e.g. the receiver mobile phone is switched off, are stored there temporarily until they can be sent.
...
\chapter{Requirements for a Key-Service}
...
% Die NAchrichten gehen über das Key Distribution Center zum Empfänger
...
\end{table} \newline
Usability is particularly important for the application. Both the actual user and other developers who write plugins for other applications should have it as easy as possible. The development of these plugins should therefore be made as easy as possible. With the signal protocol, there is a simple guide for the implementation, and once the classes for storing the secrets are written, a connection can be established with a few lines of code.\newline
...
In addition, Android 4.0 introduced the Android Keystore Provider, which is required for the application and also introduced the facial recognition feature, which makes it likely that a TEE is present, but is not a guarantee. This can only be verified after a key has been generated. If no TEE is present, a software fallback of the keystore is used.

%Nevertheless, the software keystores also offer protection against a malicious app that does not have unusual rights to access the keys of another app. However, if the attacker's app has root privileges, the app can also operate outside its own "sandbox" (a separate resource area for each app). There
% Sandbox erklären
...
%Hier viellericht noch mehr dazu erzählen
